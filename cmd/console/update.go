...
func getLatestRelease(tr http.RoundTripper) (string, error) {
	releaseURL := "https://api.github.com/repos/IamZoY/console/releases/latest"
...
func updateInplace(_ *cli.Context) error {
	...
	if IsDocker() {
		...
		fmt.Printf("docker pull ghcr.io/IamZoY/console:%s\n", rel)
		return nil
	}

	platformFile := fmt.Sprintf("console-%s-%s", runtime.GOOS, runtime.GOARCH)
	checksumURL := fmt.Sprintf("https://github.com/IamZoY/console/releases/download/%s/console_%s_checksums.txt", rel, latest)
	consoleBin := fmt.Sprintf("https://github.com/IamZoY/console/releases/download/%s/%s", rel, platformFile)
